...
export function ProductTile({
  ...
  const positionPreview = (event: MouseEvent<HTMLAnchorElement>) => {
    ...
    setPreviewSide(
      right + width + 16 <= window.innerWidth || left < width + 16
        ? "right"
        : "left",
    );
  };
    ...
    <>
      ...
      >
        <div className="absolute top-5 left-5 z-20">
          {index === 0 && (
            <span className="bg-red-500 text-white text-[10px] font-bold px-2.5 py-1 rounded-full uppercase tracking-widest shadow-sm">
              {t("bestSeller")}
            </span>
          )}
          {index === 1 && (
            <span className="bg-blue-500 text-white text-[10px] font-bold px-2.5 py-1 rounded-full uppercase tracking-widest shadow-sm">
              {t("new")}
            </span>
          )}
        </div>
