...
export function QuoteButton({
  ...
  if (variant === "hero") {
    return (
      <>
        <button
          onClick={() => setOpen(true)}
          className={`inline-flex w-full items-center justify-center gap-2 rounded-full bg-primary-foreground px-6 font-bold uppercase tracking-widest text-primary shadow-xl shadow-black/20 transition-all duration-200 hover:-translate-y-0.5 hover:bg-primary-foreground/90 hover:shadow-2xl focus-visible:outline-none focus-visible:ring-4 focus-visible:ring-primary-foreground/50 ${compact ? "py-2.5 text-[11px]" : "px-5 py-3 text-[11px] md:px-8 md:py-4 md:text-sm"}`}
        >
          {t("quoteBtn")}
        </button>
        <QuoteRequestModal
          ...
          onOpenChange={setOpen}
          source="hero"
          initialProductType={prefill?.productType}
  ...
  return (
    <>
      <button
        onClick={() => setOpen(true)}
        className={`inline-flex items-center justify-center gap-2 bg-primary text-primary-foreground px-6 py-3 rounded-full text-sm font-bold uppercase tracking-widest hover:bg-primary/90 transition shadow-lg shadow-primary/20 ${className}`}
      >
        {t("quoteBtn")}
      </button>
      <QuoteRequestModal
