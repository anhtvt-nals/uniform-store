    ...
    alternates: {
      ...
      languages: Object.fromEntries(
        routing.locales.map((l) => [
          l,
          buildCanonicalUrl(`/${l}/ve-chung-toi`),
        ]),
      ),
    },
    openGraph: {
...
export default async function AboutPage() {
  ...
  const values = [
    ...
    },
    {
      ...
      title: t("values.reputation.title"),
      desc: t("values.reputation.desc"),
    },
    {
      icon: <Users className="w-7 h-7 text-primary" />,
      title: t("values.team.title"),
      desc: t("values.team.desc"),
    },
    {
      icon: <Factory className="w-7 h-7 text-primary" />,
      title: t("values.facility.title"),
      desc: t("values.facility.desc"),
    },
  ];

  return (
    <main className="max-w-[1400px] mx-auto mt-4 flex flex-col gap-8 p-4 md:mt-6 md:p-6 lg:p-8">
      {/* Hero */}
      <div className="bg-background rounded-[32px] p-8 md:p-12 shadow-sm text-center">
        <h1 className="font-category-title text-4xl md:text-5xl text-foreground tracking-tighter mb-4">
          {t("title")}
        </h1>
      ...
      <div className="bg-background rounded-[32px] p-8 md:p-12 shadow-sm">
        <h2 className="font-category-title text-2xl md:text-3xl text-foreground tracking-tighter mb-6">
          {t("storyTitle")}
        </h2>
      ...
      <div className="py-8">
        <div className="text-center mb-10">
          <h2 className="font-category-title text-3xl text-foreground tracking-tighter mb-4">
            {t("valuesTitle")}
          </h2>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
          {values.map((item, idx) => (
            <div
              key={idx}
              className="bg-background rounded-[24px] p-8 border border-border shadow-sm hover:shadow-md transition-shadow text-center flex flex-col items-center"
            >
