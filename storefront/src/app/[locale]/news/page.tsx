...
export async function generateMetadata({
  ...
  return {
    ...
    alternates: {
      canonical: buildCanonicalUrl(`/${locale}/news`),
      languages: Object.fromEntries(
        routing.locales.map((l) => [l, buildCanonicalUrl(`/${l}/news`)]),
      ),
    },
    openGraph: {
...
async function NewsListContent({
  ...
  return (
    <main className="min-h-screen">
      {/* ── Hero ── */}
      <section className="relative bg-gradient-to-b from-primary/[0.04] via-background to-background pt-8 md:pt-12 pb-0">
        <div className="max-w-[1200px] mx-auto px-6 md:px-8">
          <div className="max-w-2xl">
            <span className="inline-block text-[11px] font-semibold text-primary uppercase tracking-[0.2em]">
              {t("featured")}
            </span>
            <h1 className="font-category-title text-4xl md:text-5xl lg:text-[3.25rem] tracking-tight leading-[1.08] mt-3">
              {t("listTitle")}
            </h1>
      ...
      <div className="max-w-[1200px] mx-auto px-6 md:px-8 py-10 md:py-12">
        {items.length === 0 ? (
          ...
          <>
            {/* Medium cards (skip featured if no active filter) */}
            {hasActiveFilter
              ? /* If filtering, start with first 2 as medium */
                items.length > 0 && (
              ...
              : /* Normal layout: skip featured, next 2 are medium */
                items.length > 1 && (
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
